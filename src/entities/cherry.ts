...
function addFallenPetals(
    ...
    for (let i = 0; i < count; i++) {
        ...
        const x = rand.nextFloat(groundXStart, groundXStart + groundWidth);
        const y = groundY + rand.nextFloat(-300,18); // Slightly vary y to not be a perfect line
        const radius = rand.nextFloat(3, 7) * scale;
        ...
        entityList.push({
            ...
            baseColor: { r: rBase, g: gBase, b: bBase, a: 1.0 },
            highlightColor: { r: Math.min(255, rBase + 15), g: Math.min(255, gBase + 15), b: Math.min(255, bBase + 15), a: 1.0 },
            type: 'leaf', // Treat as a leaf for rendering
            distFromRoot: 0, // Already on the ground
            opacity: 0.8
        });
    }
}
...
async function generateVideo() {
    ...
    if (CONFIG.photoOnly) {
        ...
        addFallenPetals(rand, entities, 30, offsetY, offsetX - 100, 500, finalScale);
    ...
    let entities: Entity[] = []; 
        ...
        addFallenPetals(rand, entities, 30, offsetY, offsetX - 100, 200, finalScale);
